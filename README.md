...
**晚间复盘**：拉全市场涨幅榜 + 涨停池 → 识别涨停股并按概念聚类找主线题材 → 从主线中选 ①连板核心股 ②首板/放量大阳突破股（换手5%~25%、主力净流入为正）→ 封板质量打分（封板时间早/零炸板/封单厚加分，多次炸板降分）→ 输出明日买入价区间、仓位、止损价。

**早间确认**：拉实时行情 → 低开<-2%剔除、高开>7%不追 → 买入区间校验（竞价价超出计划区间则计划作废）→ 成交可行性校验（开盘即封板的排队买不进，虚拟盘不买）→ 输出最终 1~2 只 + 具体股数（按本金一半内取整百股）。
...
或 Actions → 每日股票分析 → Run workflow → 选 `backtest`（**推荐云端跑**：需要拉全市场日K约5400只，10~20分钟；本地部分网络环境 push2his 不通）。

回测口径：全市场日K自建涨停日历 → 按实盘同构规则模拟（gap过滤/T+1/止损/涨停续持/佣金印花税/滑点0.3%）。已知偏差会在报告中披露（幸存者偏差、无ST/概念主线过滤、快秒板按可成交计）——**读回测数字前先读偏差说明**。

## 数据来源

东方财富公开行情接口（push2/push2his/push2ex 涨停池），无需注册、无 token。涨停与连板由涨停池官方口径 + 每日归档交叉验证。原始行情每日归档（`*_raw.json` 保留10天，口径漂移后可重算；涨停池归档长期保留供晋级率计算）。
